...
F_ym = fft(F_yx,[],2); %this is the sum over the longitude which is a fft. 

% do Gauss Legendre quadrature
ind_a = 1;
a_lm = zeros(1,((maxdeg+1)*(maxdeg+2))/2);
for l=0:maxdeg
    a_lm(ind_a:ind_a+l) = sum(P_lm{l+1}(1:l+1,:).'.*repmat(w',1,length(1:l+1)).*F_ym(:,1:l+1),1); %the repmat is hte weighting foer doing the quadrature rule
    ind_a = ind_a+l+1;
end
