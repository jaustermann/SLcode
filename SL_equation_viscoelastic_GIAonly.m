...
% Make sure this folder is located in your working directory (or add the
% complete path). 
addpath SLFunctions/
...
% Peltier, 1992). 
% 'prem' refers to the elastic/density structure (from Dziewonski and Anderson, 1981)
...
load SavedLN_EP/prem.l100C.ump4.lm4.mat %file containing the love numbers
...
for topo_it = 1:topo_it_max
    ...
    switch conv_topo
        ...
        GMSL_fo = 0; %Global mean sea level 
        ...
        for t_it = 2:length(ice_time_new) % loop over time
            ...
            GMSL_fo(t_it) = deli_lm(1)/oc_area * rho_ice/rho_water;
...
% Calculate GMSL_fo relative to present. 
GMSL_fo = -(GMSL_fo - GMSL_fo(ind_pres));

%% -------------------------------------------
% Calculate GMSL for a time-varying ocean area
% --------------------------------------------
% 
% GMSL_fo calculates the ice equivalent sea level change for a fixed ocean
% area. GMSL_tvo calculated here accounts for an 
% ocean area that changes with time following equation 1 of Lambeck et al. 
% (2014). Note that Global mean sea level (GMSL) is defined differently 
% across the community and care should therefore always be taken when 
